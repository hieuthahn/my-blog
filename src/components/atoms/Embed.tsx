...
const EmbedBlock = ({ url }: EmbedBlockProps) => {
    ...
    if (link.hostname.includes('facebook.com')) {
        ...
        const isDomainsOfEmbed = ['google.com/maps', 'mixcloud.com'].find((x) => url.includes(x));
        if (isDomainsOfEmbed) {
            ...
            if (link.hostname.includes('replit.com')) {
                ...
                (link.hostname.includes('docs.google.com') && link.pathname.includes('/forms/'))
            ) {
    ...
    return <div className="mb-4 w-full flex justify-center min-h-[250px]">{cpn}</div>;
};
