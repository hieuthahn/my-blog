...
const HeadMeta = (options: any, head: any) => {
    ...
    let siteName = settings?.site_name ?? 'HieuThahn Blog';
    let siteDescription =
        head?.description ??
        settings?.site_description ??
        'HieuThahn Blog là một blog về phát triển web, lập trình và công nghệ.';
    let siteTitle = head?.title ? `${head.title} | ${siteName}` : siteName;
    let ogImage = head?.image ?? Route.defaultCover(true);
    let author = head?.author ?? settings?.author ?? 'HieuThahn';
